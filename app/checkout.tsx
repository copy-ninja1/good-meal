...
export default function CheckoutScreen() {
  ...
  const handlePlaceOrder = async () => {
    if (!selectedAddress || !selectedPayment) {
      Alert.alert('Missing Information', 'Please select a delivery address and payment method.');
      return;
    }
      ...
      <ScrollView className="flex-1">
        ...
        <View className="bg-white p-4 mb-4">
          ...
          <View className="flex-row items-center justify-between mb-2">
            <Text className="text-gray-600">Items ({cart.length})</Text>
          ...
          <View className="border-t border-gray-200 pt-2 mt-2">
            <View className="flex-row items-center justify-between">
              ...
              <Text className="text-lg font-bold text-blue-600">{formatCurrency(totals.total)}</Text>
            </View>
          </View>
        </View>
          ...
          {selectedAddress ? (
            <View className="bg-gray-50 rounded-lg p-3">
              <View className="flex-row items-center mb-1">
                ...
                <Text className="ml-2 font-semibold capitalize">{selectedAddress.label}</Text>
              </View>
            ...
            <View className="bg-gray-50 rounded-lg p-3">
              <View className="flex-row items-center">
                ...
                <Text className="ml-2 font-semibold">
                  {selectedPayment.type === 'apple_pay' ? 'Apple Pay' :
                   selectedPayment.type === 'google_pay' ? 'Google Pay' :
                   `???? ???? ???? ${selectedPayment.last4}`}
                </Text>
              </View>
              {selectedPayment.brand && (
                <Text className="text-gray-600 capitalize ml-6">{selectedPayment.brand}</Text>
              )}
            </View>
          ) : (
            <Pressable
