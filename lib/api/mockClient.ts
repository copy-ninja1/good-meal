...
import type { Category, Product, Vendor, Order, User, Filter, SearchResult } from '../../types'
...
class MockApiClient {
  ...
  async getVendors(filters?: Filter): Promise<MockResponse<Vendor[]>> {
    ...
    if (filters?.maxDeliveryTime) {
      vendors = vendors.filter((v) => v.deliveryTime <= filters.maxDeliveryTime!)
    }
  ...
  async getProducts(vendorId?: string, category?: string): Promise<MockResponse<Product[]>> {
  ...
  async search(query: string, filters?: Filter): Promise<MockResponse<SearchResult>> {
  ...
  async updateUser(userId: string, data: Partial<User>): Promise<MockResponse<User>> {
    const user: User = {
      id: userId,
      email: data.email || '',
      name: data.name || '',
